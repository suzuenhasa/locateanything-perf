...
Where the README's headline numbers come from, and how to reproduce each one.
None of this is needed to *use* the fix — see [README.md](README.md) for that,
and [scripts/README.md](scripts/README.md) for flags and traps.

Unless a section says otherwise: RTX 3090 24 GB, bf16, no flash-attn, clean
install from this checkout. Corpus is 16 photographs and 9 page scans — book
pages, a magazine spread, an invoice.

The locate and memory tables were measured on torch 2.11.0+cu128 /
...
every stack in the version table below.
...
- [The fix, three ways](#the-fix-three-ways--and-why-stock-ooms-exactly-where-it-does)
- [Reading text](#reading-text--tiling-serving-and-torchcompile)
- [OCR text quality](#ocr-text-quality--the-decode-mode-trap)
- [The two SGLang patches](#the-two-sglang-patches-one-of-which-is-version-dependent)
- [How much slower is OCR without SGLang](#how-much-slower-is-ocr-without-sglang)
...
## Batched throughput, H100

2K photos, Detection prompt. `+ 4-D` is `apply()` alone; `+ logits` adds
...
Batch 16 and 32 need `logits_slice`, not just the attention change: at batch 8
the fp32 score matrix is still what runs out first, above it the 152,681-wide
head is. 25,600 patches is `in_token_limit` from the model's own config, and it
OOMs as shipped on both cards.
...
## Not every task speeds up the same
...
## Kernel probe, reproduced on three cards
...
## The fix, three ways — and why stock OOMs exactly where it does
...
## Reading text — tiling, serving, and `torch.compile`
...
## OCR text quality — the decode-mode trap
